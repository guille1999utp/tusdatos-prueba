...
const Login = () => {
  ...
  const onSubmit = async (data: LoginFormInputs) => {
    ...
    try {
      const resp = await authService.login(data.email, data.password); // Axios login
      console.log(resp);
      login(resp.access_token); // Guarda el token (hook personalizado)
    } catch (error: any) {
      setFormError(error.response?.data?.message || 'Error al iniciar sesión');
    } finally {
      setLoading(false);
    }
  };
    ...
    <LampContainer>
      <div className="shadow-input mx-auto w-full max-w-md rounded-none bg-white p-4 md:rounded-2xl md:p-8 dark:bg-black">
        ...
        <p className="my-2 max-w-sm text-sm text-neutral-600 dark:text-neutral-300">
          Ingresa tus datos para iniciar sesión y acceder a tu cuenta.
        </p>
        ...
        <button
          ...
          type="submit"
        >
          <a href='/register' className="text-sm text-neutral-700 dark:text-neutral-300 w-full justify-center">
            Registrarse
          </a>
          <BottomGradient />
        </button>
