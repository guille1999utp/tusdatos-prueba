...
export function DataTableDemo({ data, columns }: { data: any, columns: ColumnDef<any>[] }) {
  ...
  const fetchData = async () => {
    const respDispatch = await dispatch(getAllEvents({filters: { query: debouncedFilterTitle }}))
  ...
  React.useEffect(() => {
    if (debouncedFilterTitle !== "") {
      // Acá hacés tu petición (fetch, refetch, etc)
      fetchData()

      table.getColumn("title")?.setFilterValue(debouncedFilterTitle)
    }
